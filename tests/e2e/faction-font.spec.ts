...
test('势力角标的书法字体自带且可用', async ({ page }) => {
  ...
  const result = await page.evaluate(async () => {
    await document.fonts.load('600 13px SgsFactionScript', '魏蜀吴群晋神')
    const loaded = document.fonts.check('600 13px SgsFactionScript', '魏蜀吴群晋神')
