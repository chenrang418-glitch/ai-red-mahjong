...
- `origin/main` = `ad43fd1`（`fix(sanguosha): 甘宁【奇袭】注册了却永远用不出来`），CI 全绿，已部署。
- `feature/sanguosha` 上有一个未部署的提交（联机再来一局 / 超时接管的测试补充）。
- 部署方式：
...
| 命令 | 结果 |
|---|---|
| `npx vitest run` | 45 文件 / **438 用例** |
| `npx playwright test` | **39 通过**（Chromium 35 + WebKit 4） |
...
### 下一步
1. 联机再做一次浏览器验收，重点是两个真人同时在场的对局（这次是 1 真人 + 4 AI）。
2. 剩余武将：大乔、貂蝉。大乔【国色】要延时锦囊转化，见下方「已知简化」。
3. 剩余装备：方天画戟（多目标【杀】）、丈八蛇矛（多张牌转化）。
4. 未经用户再次明确同意，不要 commit、push 或部署。

## 之后的优先级

### 装备

需要发问的 5 件**已经做完**，都在 `src/sanguosha/engine/equipment-requests.ts`：
贯石斧、青龙偃月刀（闪抵消之后）、寒冰剑（伤害之前，替代伤害）、
麒麟弓（伤害之后，走延后发问队列）、雌雄双股剑（指定目标之后、求闪之前）。

装备用 `equip:xxx` 这种 id 直接注册进 `registerSkillRuntime`——
`getSkillRuntime` 是全局按 id 查的，不依赖武将技能表。

还剩两件：
- **方天画戟**：需要多目标【杀】，`beginSlash` 目前是单目标。
- **丈八蛇矛**：需要多张牌转化，`ViewAsOption` 目前只有单张 `cardId`，
  改成 `cardIds` 会牵动整条杀的结算管线（处理区、弃牌、奸雄定位）。
...
- **延时锦囊的转化做不了**（大乔【国色】把红桃当【乐不思蜀】）：
  判定区里的牌只记牌 id，没有「当作什么用」的标记，判定时会按牌面名字结算。
  要做得先给判定区加这个标记。普通锦囊的转化已经支持（甘宁【奇袭】）。
