...
## 下一步：蛊惑的「打出」模式（本轮没做完的）

现在只支持**使用**（出牌阶段）。**打出**（闪 / 桃 / 无懈的响应）还没接：
要在每个求牌请求的 `actionIds` 上挂一个蛊惑入口，并在响应结算中途挂起
一次多人决定，结束后把结果接回**原来那次结算**（不能新造一张牌）。
无懈尤其要接进现有的无懈链，让后面的人还能继续无懈。
