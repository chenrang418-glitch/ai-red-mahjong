...
- `origin/main` 与 `feature/sanguosha` 在本轮开始时保持同一条线；当前工作区已扩展到 56 名武将及声音、选将、艺术集 UI；
...
- 规则页按阵营整理武将；艺术集按阵营展示全部 46 张立绘并按需查看高清原图。
...
5. ~~动画~~ ✅ 已完成（`useSgsEventStage` 事件队列 + 轻重分级 + 专属皮肤）。
   **飞牌轨迹经用户明确指示暂不做**——目前出牌只表现为中央公开牌和 SVG 指向，
   没有「从牌堆/座位飞入」的完整轨迹。
6. ~~立绘~~ ✅ **当前可玩武将 56 / 56 已全部接入**（用户提供素材，2026-09-03 统一重制高清派生图）。
...
当前 56 名可玩武将均已有用户提供的立绘。座位与艺术集网格使用 480×640 WebP，
