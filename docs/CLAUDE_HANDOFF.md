...
- `origin/main` = `a639344`（`fix(sanguosha): 联机的陈旧检查太严…`），CI 全绿，已部署。
...
### 下一步
1. 方天画戟（多目标【杀】），改法见进度文档第十二批。
2. 8 人局平衡：反贼胜率仍有 76%，见下方「已知简化」。
3. 两个真人的**浏览器**验收还没做过（协议层已经有集成测试覆盖）。
...
需要发问的 5 件 + 丈八蛇矛**已经做完**，在 `src/sanguosha/engine/equipment-requests.ts`：
...
大乔【流离】和貂蝉【离间】也放在这个文件里，因为它们要动引擎内部状态。

**只剩方天画戟**：它要多目标【杀】，而 `SlashResolutionState` 是单目标的，
护驾/流离/雌雄双股剑/贯石斧/寒冰剑/麒麟弓六处都读 `targetId`。
具体改法写在 `docs/sanguosha-progress.md` 第十二批，动之前先把
`sanguosha-equipment-requests.test.ts` 和 `sanguosha-daqiao-diaochan.test.ts` 跑绿。
