...
6. **立绘**（基础设施完成，素材 0 / 25，**等用户提供素材**）：
   ...
   **不要自己去网上抓立绘**——原型阶段用过的无授权素材已按用户决定全部撤除。
   ...
   用户拿到素材后照 `docs/sanguosha-portraits.md` 接入。那份文档里最容易被忽略、
   一旦搞错就得整批返工的两条，直接抄在这里：

   - **脸宽必须 ≥ 画面宽度的 25%**，所以只能是半身像，不能是全身站姿。
     竖屏最小座位只有 90px 宽，脸占 15% 时屏幕上只有 16px，认不出是谁。
   - **PC 自己座位（280×120）只能看到 3:4 源图纵向的 32%**，
     所以脸周围必须留余量，头顶留白 ≥ 8%，贴边的构图一裁就没。

   规格一句话版：720×960（3:4）半身像，脸在上部、宽度占画面 25% 以上，
   背景简单无文字，导出 360px 宽静态 WebP，**文件名 = characterId**（拼错会静默回退）。
