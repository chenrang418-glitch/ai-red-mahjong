...
- `origin/main` = `c672ef4`（历史基线，未变）
- `feature/sanguosha` 领先 `main` 若干提交，**尚未推送**。最新一条见 `git log --oneline -1`，
  本次交接时为 `1794c29 feat(sanguosha): 黄忠【烈弓】，并抽出「不可闪避」的公共入口`
  之后的首页计数改动。**push 前先跑三层验证。**
...
| 命令 | 结果 |
|---|---|
| `npx vitest run` | 63 文件 / **590 用例 + 1 todo** |
...
**已完成 4 名**（典韦、魏延、庞德、黄忠），**剩 3 名**：

| 武将 | 技能 | 卡在哪 / 可复用什么 |
|---|---|---|
| 颜良文丑（火） | 双雄 | 需要「回合内临时状态」+ 判定结果决定转化。临时状态用 `player.turnUsedSkills` 或 `marks`（`TurnEnd` 已统一清理）；转化走 `SkillRuntime.viewAs` |
| 小乔（风） | 天香、红颜 | 红颜需要一层**等效花色**（引擎里直接读 `.suit` 的地方只有 9 处，逐个改掉）；天香是伤害转移，`BeforeDamage`/`DamageInflicted` 这两个可取消、可改数值的时机已经具备 |
| 夏侯渊（风） | 神速 | 需要**跳过阶段**+ 无实体【杀】。阶段状态机在 `engine/phase.ts`，无实体牌目前没有通用路径（貂蝉离间是拿被弃的牌当载体绕过去的），这一个改动最大 |
...
**尚未登记的武将其立绘已经在仓库里躺着，武将一落地就自动生效**，不需要再找素材。
