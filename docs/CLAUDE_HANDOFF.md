...
## 用户当前的待办清单（2026-09-01 给出，尚未开始）

按用户原话排的优先级，前三项已完成，其余待做：

1. ~~单机退出结算界面~~ ✅ 已完成（退出确认 + 结算名单 + 再来一局）
2. ~~选角色界面竖屏超出屏幕~~ ✅ 已完成（底部面板封顶 + 内部滚动）
3. ~~对局中选项多时超出屏幕下方~~ ✅ 同上，根因是同一个
4. **局内词条查看**（未开始）：点技能名 / 牌名 / 武将名就能看到说明，
   **不要跳规则界面**，要就地弹出。
   数据源已经就绪且是单一来源：武将技能在 `data/characters/*.ts` 的 `skills[].description`，
   牌面说明在 `data/ruleset-v1/card-info.ts`。只差一个点击弹层。
5. **动画**（未开始）：出牌、伤害、判定的过渡。
6. **立绘**（未开始，**需要用户先定方案**）：我不能抓现成的三国杀立绘（版权），
   可选项是程序化生成的抽象牌面（印章风格），或者由用户提供素材。
7. **新增角色**（未开始）：标准包 26 名已满，再加要进军争 / 风火林山等扩展包。
