...
- `origin/main` = `b3a2296`（`feat(sanguosha): 接通联机房间与前端`），CI 全绿，已部署。
- `feature/sanguosha` 当前也基于 `b3a2296`，其上有管理员三国杀房间支持和吕蒙【克己】的**未提交工作树**；不要 reset/clean，也不要直接切分支。
- 生产部署 Actions run：`33467355998`，Worker、Pages、测试与 smoke 全部成功；`https://crplay.cn/api/health` 已实测正常。
- 部署方式：
...
| 命令 | 结果 |
|---|---|
| `npx vitest run` | 43 文件 / **404 用例**（本轮预计值，最终全量回归后以实际输出为准） |
...
3. 继续剩余 3 将与 5 件装备，不要把未完整角色登记进武将包。
4. 管理员房间页已能区分并解散麻将/三国杀房间；继续做前端真实浏览器验收。
5. 本轮未提交增量仍需完整回归；未经用户再次明确同意，不要 commit、push 或部署。
