...
1. 实现**需要 Request 的装备特效**：八卦阵（判定代闪）、麒麟弓（弃马）、
...
- Phase C 仍缺：**需要 Request 的装备特效**（八卦阵/麒麟弓/青龙/贯石斧/寒冰/方天/
  雌雄/丈八），以及标准 25 将技能
...
| 命令 | 结果 |
| --- | --- |
| `npm run test:run` | **283 通过 / 32 个文件**（麻将 188 + 三国杀 89 + 门户 6） |
| `npm run test:sanguosha` | **89 通过 / 11 个文件** |
| `npm run typecheck` | 通过 |
| `npm run typecheck:online` | 通过 |
