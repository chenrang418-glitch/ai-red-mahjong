...
- 房间 AI 固定为平衡型、凡人、猴急；真人掉线 30 秒或操作超时 30 秒后，由真人波动型、凡人、猴急的 AI 接管。重连后可以取消托管。切换托管只影响自己的座位，不会重置别人的操作倒计时，也不会延长抢牌窗口或清除别人已经做出的响应。
...
自动测试覆盖胡牌、七对、红中限制、112 张守恒、六码隔离、三种杠分、自摸抓码、余额封顶、无限积分、庄家轮换、AI 暗牌隔离，以及联机房间、手牌脱敏、AI 托管、托管切换不影响他人计时、聊天上限和操作幂等。联机冒烟测试会在本机启动隔离的 Worker、D1 和 Durable Object，验证昵称登录、WebSocket、创建房间、AI 补位和快捷聊天的完整链路。
