...
import { huashenEligibleSkills } from '../engine/huashen'
...
const props = withDefaults(defineProps<{
  ...
  selected?: boolean
  threatened?: boolean
  effect?: 'damage' | 'recover' | 'dodge' | 'skill' | 'skill-target' | null
  status?: 'online' | 'offline' | 'trustee' | 'connecting' | null
  hint?: string
  displayName?: string
  /** 认了这名角色当「麻麻」的牛来们（显示名）。公开信息，空数组表示不是任何人的麻麻。 */
  mamaOwners?: readonly string[]
}>(), { active: false, targeting: false, selectable: false, selected: false, threatened: false, effect: null, status: null, hint: '', displayName: '', mamaOwners: () => [] })
...
const huashenPanelOpen = ref(false)
...
const marks = computed(() => (props.player.alive
  ? Object.entries(props.player.marks ?? {})
    ...
    .map(([key, value]) => ({ key, label: VISIBLE_MARKS[key], value }))
  // 阵亡角色身上的标记留在状态里没坏处（引擎在 Death 时角色已经不算存活，
  // 技能触发器跑不到），但座位卡上不该再挂着
  : []))
const hasStates = computed(() => props.player.chained || props.player.faceDown || !props.player.alive
  || props.mamaOwners.length > 0 || marks.value.length > 0
  || props.player.characterSkillsDisabled || Boolean(props.player.huashen?.activeCharacterId)
...
const activeHuashen = computed(() => {
  const state = props.player.huashen
  if (!state?.activeCharacterId) return null
  const activeCharacter = getCharacter(state.activeCharacterId)
  const activeSkill = props.player.skills.find((skill) => skill.id === state.activeSkillId)
  return {
    characterName: activeCharacter?.name ?? state.activeCharacterId,
    skillName: activeSkill?.name ?? state.activeSkillId ?? '无',
    kingdom: factionDefinition(props.player.kingdom)?.name ?? '未知',
    gender: GENDER_TEXT[props.player.gender ?? ''] ?? '未知',
  }
})
const ownedHuashen = computed(() => (props.player.huashen?.ownedCharacterIds ?? []).map((characterId) => ({
  characterId,
  character: getCharacter(characterId),
  skills: huashenEligibleSkills(characterId),
})))
/**
...
const factionVars = computed(() => {
  const faction = factionDefinition(props.player.kingdom)
  return faction ? { '--seat-faction-color': faction.color } : undefined
})
</script>
...
<template>
  <article
    class="sgs-seat"
    :class="{
      'sgs-seat--active': active, 'sgs-seat--targeting': targeting, 'sgs-seat--selected': selected, 'sgs-seat--selectable': selectable,
  ...
  >
    <button v-if="targeting" type="button" class="sgs-seat__target-hitbox" :disabled="!selectable" :aria-label="selectable ? `选择${player.nickname}为目标` : `${player.nickname}不是合法目标`" @click="emit('select', player.id)"></button>
    <!--
      座位是四层：势力底色 → 立绘 → 渐变遮罩 → 信息。
      立绘用独立 <img> 而不是 background-size:cover，因为位置、缩放、灰度、
      阵亡态这些以后都要分别控制，背景图给不了这个自由度。
    -->
    <div class="sgs-seat__portrait" :style="factionVars" aria-hidden="true"><span v-if="!portrait">{{ initials }}</span></div>
    ...
    <div v-if="player.kingdom" class="sgs-seat__corner">
      <SgsFactionBadge :faction="player.kingdom" />
      <span v-if="active" class="sgs-seat__turn">行动中</span>
    </div>
    ...
    <div v-if="player.judgingArea.length || hasStates" class="sgs-seat__states">
      ...
      <span
        v-for="mark in marks"
        :key="mark.key"
        class="sgs-seat__mark-chip"
        :aria-label="`${mark.label} ${mark.value} 枚`"
        :title="`${mark.label} ${mark.value} 枚`"
      >{{ mark.label }}{{ mark.value }}</span><span
        v-if="mamaOwners.length"
        class="sgs-seat__mama-chip"
        :aria-label="`${mamaOwners.join('、')}的麻麻`"
        :title="`${mamaOwners.join('、')}的麻麻`"
      >麻麻</span><span v-if="activeHuashen" class="sgs-seat__huashen-chip" :title="`当前化身：${activeHuashen.characterName}；当前技能：${activeHuashen.skillName}；势力：${activeHuashen.kingdom}；性别：${activeHuashen.gender}`">化身 {{ activeHuashen.characterName }}·{{ activeHuashen.skillName }}·{{ activeHuashen.kingdom }}·{{ activeHuashen.gender }}</span><button v-if="player.id === viewerId && ownedHuashen.length" type="button" class="sgs-seat__huashen-library" @click.stop="huashenPanelOpen = true">化身库</button><span v-if="player.characterSkillsDisabled">技能失效</span><button v-if="player.chained" type="button" @click.stop="glossary?.open(ruleGlossary('chained'))">横置</button><span v-if="player.faceDown">翻面</span><span v-if="!player.alive">阵亡</span><span v-if="status === 'offline'">离线</span><span v-if="status === 'trustee'">托管</span><span v-if="status === 'connecting'">连接中</span>
    </div>
    <div v-if="(player.skills ?? []).length" class="sgs-seat__skills"><button v-for="skill in (player.skills ?? [])" :key="skill.id" type="button" @click.stop="glossary?.open(skillGlossary(skill.id))">{{ skill.name }}</button></div>
    <p v-if="hint" class="sgs-seat__hint">{{ hint }}</p><span v-if="selected" class="sgs-seat__target-mark">目标</span>
    <Teleport to="body">
      <div v-if="huashenPanelOpen && player.id === viewerId" class="sgs-huashen-panel__backdrop" @click.self="huashenPanelOpen = false">
        <section class="sgs-huashen-panel" role="dialog" aria-modal="true" aria-label="化身库">
          <header><div><strong>化身库</strong><small>仅你可见</small></div><button type="button" aria-label="关闭化身库" @click="huashenPanelOpen = false">×</button></header>
          <div class="sgs-huashen-panel__list">
            <article v-for="entry in ownedHuashen" :key="entry.characterId" :class="{ active: entry.characterId === player.huashen?.activeCharacterId }">
              <div><strong>{{ entry.character?.name ?? entry.characterId }}</strong><small>{{ factionDefinition(entry.character?.kingdom)?.name ?? '未知' }} · {{ GENDER_TEXT[entry.character?.gender ?? ''] ?? '未知' }}</small></div>
              <span v-if="entry.characterId === player.huashen?.activeCharacterId">当前化身</span>
              <p>{{ entry.skills.map((skill) => skill.name).join('、') || '无可声明技能' }}</p>
            </article>
          </div>
        </section>
      </div>
...
<style scoped>
.sgs-seat{position:relative;min-width:0;height:100%;overflow:hidden;display:flex;flex-direction:column;justify-content:flex-end;gap:3px;padding:7px 8px;border:1px solid #415249;border-radius:12px;background:#14241d;color:#eee6d2;box-shadow:0 6px 18px rgba(0,0,0,.28);transition:opacity .18s,border-color .18s,transform .18s}.sgs-seat__target-hitbox{position:absolute;inset:0;z-index:1;border:0;border-radius:inherit;background:transparent;cursor:crosshair}.sgs-seat__portrait{position:absolute;inset:0;display:grid;place-items:center;opacity:.8;background:radial-gradient(circle at 60% 30%,rgba(181,148,79,.22),transparent 45%),linear-gradient(145deg,#344b3e,#14231d)}.sgs-seat__portrait span{color:rgba(235,217,166,.2);font:900 clamp(38px,5vw,76px)/1 KaiTi,serif;transform:rotate(-7deg)}.sgs-seat__shade{position:absolute;inset:0;background:linear-gradient(90deg,rgba(8,16,12,.94),rgba(8,16,12,.7) 58%,rgba(8,16,12,.28))}.sgs-seat>:not(.sgs-seat__portrait,.sgs-seat__art,.sgs-seat__shade,.sgs-seat__target-hitbox){position:relative;z-index:2}.sgs-seat--selectable{border-color:#d6bd69;box-shadow:0 0 0 2px rgba(214,189,105,.28),0 0 18px rgba(214,189,105,.25)}.sgs-seat--selectable:not(.sgs-seat--selected){animation:seat-candidate 1.4s ease-in-out infinite}.sgs-seat--active{border-color:#e8c66d;box-shadow:0 0 0 1px rgba(232,198,109,.45),0 0 22px rgba(232,198,109,.32);animation:seat-active 1.8s ease-in-out infinite}.sgs-seat--selected{border-color:#f06f5c;box-shadow:0 0 0 3px rgba(240,111,92,.55),0 0 24px rgba(240,85,70,.3)}.sgs-seat--threatened,.sgs-seat--effect-damage{border-color:#ef6559;animation:seat-hit .48s ease}.sgs-seat--effect-recover{border-color:#68d191;animation:seat-recover .65s ease}.sgs-seat--effect-dodge{border-color:#dce8d4}.sgs-seat--effect-skill{border-color:#b89bf0}.sgs-seat--dead{opacity:.76}.sgs-seat--chained:after{content:'⛓';position:absolute;right:5px;bottom:4px;z-index:2;color:#d0b865;font-size:14px}.sgs-seat__header{display:flex;align-items:center;gap:4px;min-width:0}.sgs-seat__header strong{min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:11px}button{font:inherit}.sgs-seat__identity,.sgs-seat__general,.sgs-seat__meta button,.sgs-seat__equipment button,.sgs-seat__states button,.sgs-seat__skills button{border:0;padding:0;background:transparent;color:inherit;cursor:help}.sgs-seat__identity{flex:none;padding:1px 5px;border-radius:4px;background:#2b3831;color:#9cac9f;font-size:9px}.sgs-seat__identity--lord{background:#6a4a1c;color:#ffd98a}.sgs-seat__identity--rebel{background:#5c2622;color:#ffb3aa}.sgs-seat__identity--loyalist{background:#21432f;color:#a6e0bb}.sgs-seat__identity--renegade{background:#3d3151;color:#cbb6ee}.sgs-seat__turn{margin-left:auto;padding:1px 4px;border-radius:4px;background:#806226;color:#ffe39a;font-size:8px;white-space:nowrap}.sgs-seat__body{display:grid;gap:2px}.sgs-seat__general{justify-self:start;color:#ead28b;font-weight:800;font-size:13px}.sgs-seat__hp{display:flex;align-items:center;gap:1px;color:#e76054;font-size:12px;line-height:1}.sgs-seat__hp .empty{color:#8d8375}.sgs-seat__hp small{margin-left:3px;color:#d8cfc0;font-size:8px}.sgs-seat__meta{display:flex;gap:7px;color:#a5b3aa;font-size:9px}.sgs-seat__meta button{text-decoration:underline dotted;text-underline-offset:2px}.sgs-seat__equipment,.sgs-seat__piles{display:flex;gap:3px;min-width:0;overflow:hidden}.sgs-seat__piles button{padding:1px 4px;border:1px solid rgba(215,150,120,.4);border-radius:3px;background:rgba(60,26,22,.7);color:#f0b9a6;font-size:8px;cursor:help}.sgs-seat__equipment button{min-width:0;display:flex;gap:3px;padding:1px 3px;border:1px solid rgba(157,141,96,.3);border-radius:4px;background:rgba(8,14,11,.6);font-size:8px}.sgs-seat__equipment small{color:#8e9b92}.sgs-seat__equipment span{overflow:hidden;text-overflow:ellipsis;white-space:nowrap;color:#d7c795}.sgs-seat__states,.sgs-seat__skills{display:flex;gap:3px;flex-wrap:wrap;font-size:8px}.sgs-seat__states span,.sgs-seat__states button{flex:none;padding:1px 3px;border-radius:3px;background:#3a2926;color:#f1afa5}.sgs-seat__huashen-chip{flex:1 1 100%!important;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.sgs-seat__huashen-library{background:#4b3b65!important;color:#e1d2ff!important;cursor:pointer!important}.sgs-seat__judge-chip{min-width:13px;text-align:center;background:#7d2b23;color:#ffd9d2;font-weight:800}.sgs-seat__mama-chip{background:#4a3a6b;color:#dcc8ff;font-weight:800}.sgs-seat__mark-chip{background:#5a4520;color:#f4d79a;font-weight:800}.sgs-seat__skills button{padding:1px 3px;border:1px solid rgba(210,183,106,.25);border-radius:3px;color:#cdbd8e}.sgs-seat__hint{margin:0;color:#f2d47e;font-size:8px}.sgs-seat__target-mark{position:absolute!important;right:5px;top:24px;padding:2px 5px;border-radius:8px;background:#a83e35;color:white;font-size:8px;font-weight:800}.sgs-huashen-panel__backdrop{position:fixed!important;inset:0;z-index:120!important;display:grid;place-items:center;padding:16px;background:rgba(2,7,5,.88)}.sgs-huashen-panel{width:min(680px,100%);max-height:min(88dvh,820px);display:grid;grid-template-rows:auto minmax(0,1fr);overflow:hidden;border:1px solid #806c42;border-radius:18px;background:#101913;color:#eee6d2;box-shadow:0 24px 80px #000}.sgs-huashen-panel>header{display:flex;align-items:center;justify-content:space-between;padding:12px 14px;border-bottom:1px solid #3d4b42}.sgs-huashen-panel>header div{display:grid}.sgs-huashen-panel>header strong{color:#f2d994;font-size:18px}.sgs-huashen-panel>header small{color:#9aa79f}.sgs-huashen-panel>header button{width:38px;height:38px;border:0;background:transparent;color:#ddd4c0;font-size:28px;cursor:pointer}.sgs-huashen-panel__list{min-height:0;overflow:auto;display:grid;gap:8px;padding:12px;overscroll-behavior:contain}.sgs-huashen-panel__list article{display:grid;grid-template-columns:1fr auto;gap:4px 10px;padding:10px;border:1px solid #39483f;border-radius:10px;background:#17231d}.sgs-huashen-panel__list article.active{border-color:#c8a955;background:#252617}.sgs-huashen-panel__list article div{display:grid}.sgs-huashen-panel__list article small{color:#96a49b}.sgs-huashen-panel__list article>span{align-self:start;padding:2px 6px;border-radius:5px;background:#6b5121;color:#f7dc92;font-size:11px}.sgs-huashen-panel__list article p{grid-column:1/-1;margin:0;color:#cdbd8e;font-size:13px;line-height:1.5}@keyframes seat-active{50%{box-shadow:0 0 0 2px rgba(232,198,109,.55),0 0 28px rgba(232,198,109,.4)}}@keyframes seat-candidate{50%{border-color:#ffe39a}}@keyframes seat-hit{30%{transform:translateX(-4px)}60%{transform:translateX(4px)}}@keyframes seat-recover{50%{box-shadow:0 0 28px rgba(81,210,132,.62)}}@media(max-width:820px){.sgs-seat{padding:5px 6px;border-radius:9px}.sgs-seat__general{font-size:11px}.sgs-seat__hp{font-size:10px}.sgs-seat__header strong{font-size:9px}.sgs-huashen-panel__backdrop{place-items:end center;padding:10px}.sgs-huashen-panel{width:100%;max-height:78dvh;border-radius:16px 16px 8px 8px}}@media(prefers-reduced-motion:reduce){.sgs-seat{animation:none!important;transition:none!important}}
.sgs-seat__target-hitbox{z-index:3}.sgs-seat button:not(.sgs-seat__target-hitbox){position:relative;z-index:4}
/* 目标选择期间整张座位是唯一点击面；词条按钮仍显示，但暂时不抢触摸。 */
.sgs-seat--targeting .sgs-seat__target-hitbox{z-index:8}.sgs-seat--targeting button:not(.sgs-seat__target-hitbox){pointer-events:none}.sgs-seat--targeting:not(.sgs-seat--selectable){opacity:.56}.sgs-seat--targeting .sgs-seat__target-hitbox:disabled{cursor:not-allowed}.sgs-seat--effect-skill-target{border-color:#8da9e8;box-shadow:0 0 16px rgba(110,145,225,.28)}
...
.sgs-seat--has-art .sgs-seat__states .sgs-seat__mark-chip { background: rgba(90, 69, 32, .95); color: #ffe6b4; text-shadow: 0 1px 2px rgba(0, 0, 0, .9); }
...
/* 右上角只放公开状态，脱离文档流，绝不改变座位高度或点击区域。 */
.sgs-seat .sgs-seat__corner {
  position: absolute; z-index: 7; top: 6px; right: 6px; display: grid; justify-items: end; gap: 2px;
  pointer-events: none;
}
.sgs-seat__header { padding-right: 27px; }
.sgs-seat__portrait {
  background-color: var(--seat-faction-color, #344b3e);
  background-image: radial-gradient(circle at 60% 30%, rgba(181,148,79,.22), transparent 45%), linear-gradient(145deg, transparent, #14231d 88%);
}
.sgs-seat__target-mark { top: 42px; }
@media (max-width: 820px), (orientation: landscape) and (max-height: 500px) {
  .sgs-seat .sgs-seat__corner { top: 4px; right: 4px; }
  .sgs-seat__header { padding-right: 23px; }
  .sgs-seat__target-mark { top: 36px; }
}
</style>
