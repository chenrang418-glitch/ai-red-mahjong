...
const KINGDOM_TEXT: Record<string, string> = { wei: '魏', shu: '蜀', wu: '吴', qun: '群' }
...
const activeHuashen = computed(() => {
  ...
  return {
    ...
    kingdom: KINGDOM_TEXT[props.player.kingdom ?? ''] ?? '未知',
...
<template>
  ...
  >
    ...
    -->
    <div class="sgs-seat__portrait" :class="`sgs-seat__portrait--${player.kingdom ?? 'unknown'}`" aria-hidden="true"><span v-if="!portrait">{{ initials }}</span></div>
        ...
        <section class="sgs-huashen-panel" role="dialog" aria-modal="true" aria-label="化身库">
          ...
          <div class="sgs-huashen-panel__list">
            <article v-for="entry in ownedHuashen" :key="entry.characterId" :class="{ active: entry.characterId === player.huashen?.activeCharacterId }">
              <div><strong>{{ entry.character?.name ?? entry.characterId }}</strong><small>{{ KINGDOM_TEXT[entry.character?.kingdom ?? ''] ?? '未知' }} · {{ GENDER_TEXT[entry.character?.gender ?? ''] ?? '未知' }}</small></div>
